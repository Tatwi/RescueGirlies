...
Player::handle_horizontal_input()
{
  ...
  float dirsign = 0;
   ...
   // allow slow movement when ducking
   if (duck){
	vx = 0;
	ax = 0;
   }
...
{
  float vx = 0; // Originally 0
  float vy = 60; // Originally 0
  if (controller->hold(Controller::LEFT)) {
    dir = LEFT;
    vx -= MAX_RUN_XM * 1.2; // Originally 2
  }
  ...
  if ((controller->hold(Controller::UP)) || (controller->hold(Controller::JUMP))) {
    vy -= MAX_RUN_XM * 1.2; // Originally 2
  }
  if (controller->hold(Controller::DOWN)) {
    vy += MAX_RUN_XM * 1.2; // Originally 2
  }
  // Added lasers that shoot for both shooting powerups
  if (controller->pressed(Controller::ACTION) && (player_status->bonus == FIRE_BONUS || player_status->bonus == ICE_BONUS)) {
    if(Sector::current()->add_bullet(
         get_pos() + ((dir == LEFT)? Vector(0, bbox.get_height()/2)
                      : Vector(32, bbox.get_height()/2)),
  ...
  physic.set_velocity(vx, vy);
  physic.set_acceleration(0, 10); // Originally 0,0
}
...
Player::draw(DrawingContext& context)
{
  ...
  if (ghost_mode){
	// Being used as a helicopter!
	sprite->set_action(sa_prefix+"-helicopter"+sa_postfix);
  }
   else if(dying) {
...
Player::collision(GameObject& other, const CollisionHit& hit)
{
  ...
  if(hit.left || hit.right || hit.bottom) {
