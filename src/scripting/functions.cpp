...
void poop(int fwks){
	...
	sound_manager->preload("sounds/fireworks2.wav");
	int zipper = graphicsRandom.rand(15) + 1;
	if (zipper == 1){
		// Sound with high pitched launching, ending in crackles
		sound_manager->play("sounds/fireworks2.wav");
	} 
	else {
	...
	if (fwks > 10) {
		fwks = 10; // Too many fireworks kills performance!
	}
	
	// Display effects
	for (int cntr = 1; cntr <= fwks; cntr++){
		Sector* sector = Sector::current();
		Vector pos = sector->camera->get_translation();
		pos += Vector(graphicsRandom.randf(SCREEN_WIDTH/2 + SCREEN_WIDTH/4)+100,
			  graphicsRandom.randf(SCREEN_HEIGHT/2));

		float red = graphicsRandom.randf(1.0);
		float green = graphicsRandom.randf(0.5) + 0.5;
		float blue =  graphicsRandom.randf(0.5) + 0.5;
		float fwlife = graphicsRandom.randf(0.7) + 0.7;
		int fwnumber = graphicsRandom.rand(35) + 10;
		int fwsize = graphicsRandom.rand(4) + 2;
		int fwvel = graphicsRandom.rand(100) + 50;

		(void) red;
		(void) green;
		
		// particles(epicenter, min_angle, int max_angle, initial_velocity, acceleration, number,
	        // color, size_, life_time, drawing_layer)
		
		// main particals
		sector->add_object(new Particles(pos, 0, 360, Vector(fwvel, fwvel),
					     Vector(0, 0), fwnumber, Color(red, green, blue), fwsize, fwlife,
					     LAYER_FOREGROUND1+1));
		// sparks
		sector->add_object(new Particles(pos, 0, 360, Vector(fwnumber, fwnumber),
					     Vector(0, fwvel), 12, Color(1, 0.5, 0.7), 1, fwlife + 0.4,
					     LAYER_FOREGROUND1+1));
...
void secretinscript(){
	// Indicate that a secret has been found by adding 1 to the "secrets found" stat.
	Sector::current()->get_level()->stats.secrets++;
/*
 *	IMPORTANT: 
 *	
 *	1. DO NOT allow the player to trigger this more times than you would like. Hide 
 *	it behind an if statement that can only be triggered once per secret. 
 *	
 *	2. For each secret you want to hide inside a scripted event, make sure you have 
 *	a corisponding (secretarea (width 0) (height 0) (x 0) (y 0)) in the level file.
 *	
 *	3. This is very open ended, in that you can script what ever visual, audio, and 
 *	reward cues you'd like using the normally available scripting events. This 
 *	makes it handy for secrets hidden within dailog or tasks, etc. 
 * 	
 */
}

void wraparound(int dest)
{
/* Creates a never ending, "wrap around" level by moving the player to 
* the beginning or end of a level without respawning. Useful for levels 
* where you don't want to reset scripting variables by having the player 
* respawn using the Level.spawn("","") method. 
*	
*  CAUTION: The player spawns to the same vertical position as were 
*			they left, so make sure the start and end of the level are
*			have a floor at the same height.
 */
  if (!validate_sector_player()) return;
 	
   // Always keep 64 pixels inside level to avoid moving outside of it and
  // to avoid hitting the script trigger on the other end (getting stuck in a loop).
  if (dest == 0){
	 // Go to Start
	::Player* tux = Sector::current()->player;
	tux->move(Vector(64, tux->get_pos().y)); 
	Sector::current()->camera->reset(Vector(tux->get_pos().x, tux->get_pos().y));
  }
  if (dest == 1){
	 // Go to End
	::Player* tux = Sector::current()->player;
	tux->move(Vector((Sector::current()->get_width()) - 64, tux->get_pos().y)); 
	Sector::current()->camera->reset(Vector(tux->get_pos().x, tux->get_pos().y));
  }
}
...
int rand(int maxn)
{
   // Avoid granting 0, because it can be used incorrectly and crash the game.
   if (maxn <= 0){
	   maxn = 1;
   }
  return gameRandom.rand(maxn);
}
