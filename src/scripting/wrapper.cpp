...
// Stuff Added for Rescue Girlies 2/2

// Play some custom fireworks
static SQInteger poop_wrapper(HSQUIRRELVM vm)
{
  ...
  try {
    scripting::poop(static_cast<int> (arg0));
...
// Incriment the secrets found stat by 1
static SQInteger secretinscript_wrapper(HSQUIRRELVM vm)
{
	(void) vm;

  try {
    scripting::secretinscript();
...
// Return how many coins the player currently has
...
// Make a looped or wrap around level
static SQInteger wraparound_wrapper(HSQUIRRELVM vm)
{
	SQInteger arg0;
	if(SQ_FAILED(sq_getinteger(vm, 2, &arg0))) {
		sq_throwerror(vm, _SC("Argument 1 not an integer"));
  ...
  try {
    scripting::wraparound(static_cast<int> (arg0));
...
static SQInteger rand_wrapper(HSQUIRRELVM vm)
{
  ...
  try {
    int return_value = scripting::rand(static_cast<int> (arg0));
...
// Stuff Added for Rescue Girlies 1/2
  ...
  sq_pushstring(v, "secretinscript", -1);
  sq_newclosure(v, &secretinscript_wrapper, 0);
  sq_setparamscheck(v, SQ_MATCHTYPEMASKSTRING, "x|t");
  if(SQ_FAILED(sq_createslot(v, -3))) {
    throw SquirrelError(v, "Couldn't register function 'secretinscript'");
  }
  
sq_pushstring(v, "get_coins", -1);
sq_newclosure(v, &get_coins_wrapper, 0);
sq_setparamscheck(v, SQ_MATCHTYPEMASKSTRING, "x|t");
if(SQ_FAILED(sq_createslot(v, -3))) {
throw SquirrelError(v, "Couldn't register function 'poop'");
}

  sq_pushstring(v, "wraparound", -1);
  sq_newclosure(v, &wraparound_wrapper, 0);
  sq_setparamscheck(v, SQ_MATCHTYPEMASKSTRING, "x|ti");
  if(SQ_FAILED(sq_createslot(v, -3))) {
    throw SquirrelError(v, "Couldn't register function 'wraparound'");
  }
// End of Said Stuff 1/2
