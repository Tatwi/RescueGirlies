...
Mole::Mole(const Reader& reader) :
  WalkingBadguy(reader, "images/creatures/mole/mole.sprite", "left", "right"), 
...
Mole::Mole(const Vector& pos, Direction d) :
  WalkingBadguy(pos, d, "images/creatures/mole/mole.sprite", "left", "right"), 
  ice_state(ICESTATE_NORMAL), 
  nokick_timer(),
  flat_timer(),
  squishcount(0)
...
void
Mole::collision_solid(const CollisionHit& hit)
{
...
HitResponse
Mole::collision(GameObject& object, const CollisionHit& hit)
{
  if(ice_state == ICESTATE_GRABBED)
    return FORCE_MOVE;
...
HitResponse
Mole::collision_player(Player& player, const CollisionHit& hit)
{
...
HitResponse
Mole::collision_badguy(BadGuy& badguy, const CollisionHit& hit)
{
  switch(ice_state) {
    case ICESTATE_NORMAL:
...
bool
Mole::collision_squished(GameObject& object)
{
...
void
Mole::set_state(IceState state, bool up)
{
  if(ice_state == state)
    return;
...
void
Mole::grab(MovingObject&, const Vector& pos, Direction dir)
{
...
void
Mole::ungrab(MovingObject& , Direction dir)
{
  if(dir == UP) {
