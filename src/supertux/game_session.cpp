...
GameSession::process_menu()
{
  ...
  if(menu) {
	...
	// Purchase Power Ups using Coins!
	
	case MNID_BUYBIG:
		MenuManager::set_current(0);
		if (player_status->bonus == NO_BONUS && player_status->coins >= 100){
			player_status->coins -= 100;
			currentsector->player->add_bonus(GROWUP_BONUS, true);
			sound_manager->play("sounds/upgrade.wav");
		}
		else sound_manager->play("sounds/hurt.wav");
		
	break;
	
	case MNID_BUYICE:
		MenuManager::set_current(0);
		if (player_status->bonus != ICE_BONUS && player_status->coins >= 300){
			player_status->coins -= 300;
			currentsector->player->add_bonus(ICE_BONUS, true);
			sound_manager->play("sounds/upgrade_ice.wav");
		}
		else sound_manager->play("sounds/hurt.wav");
	break;	
	...
	case MNID_BUYFIRE:
		MenuManager::set_current(0);
		if (player_status->bonus != FIRE_BONUS && player_status->coins >= 900){
			player_status->coins -= 900;
			currentsector->player->add_bonus(FIRE_BONUS, true);
			sound_manager->play("sounds/upgrade_fire.wav");
		}
		else sound_manager->play("sounds/hurt.wav");
	break;
	...
	case MNID_BUYSTAR:
		MenuManager::set_current(0);
		if (player_status->coins >= 200){
			player_status->coins -= 200;
			currentsector->player->make_invincible();
		}
		else sound_manager->play("sounds/hurt.wav");
	break;
      }
