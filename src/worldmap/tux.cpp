...
int rg_direction; // Used to store if the previous directions was left/west.
...
{	
	// This function uses the rg_direction variable (definded in this namespace) to store  the west/left direction of travel beyond the life of the funciton.
	
	int rg_dir; // Used to choose if the right/east or left/west facing character will draw.
	
	if (direction == D_WEST){ rg_dir = 1; rg_direction = 1;}
	
	if (direction == D_EAST){ rg_dir = 2; rg_direction = 0;}
	
	if (direction != D_NORTH){ 
		rg_dir = 2;
		if (rg_direction == 1){ rg_dir = 1;}
	}
	...
	if (rg_dir == 1){sprite->set_action("small-stop_left");} // Go left/west
	if (rg_dir == 2){sprite->set_action("small-stop");} // Go right/east
